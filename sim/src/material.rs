...
pub const PALETTE: [MatInfo; 8] = [
    MatInfo { name: "regolith",  hardness: 1.0, cohesion: 0.35, albedo: [0.42, 0.34, 0.24], roughness: 0.85 },
    MatInfo { name: "sediment",  hardness: 0.7, cohesion: 0.25, albedo: [0.38, 0.30, 0.18], roughness: 0.80 },
    MatInfo { name: "clay",      hardness: 1.2, cohesion: 0.55, albedo: [0.40, 0.28, 0.20], roughness: 0.70 },
    MatInfo { name: "sandstone", hardness: 2.4, cohesion: 0.70, albedo: [0.55, 0.42, 0.28], roughness: 0.65 },
    MatInfo { name: "basalt",    hardness: 4.0, cohesion: 0.90, albedo: [0.22, 0.22, 0.24], roughness: 0.55 },
    MatInfo { name: "ferrous ore", hardness: 3.2, cohesion: 0.80, albedo: [0.35, 0.22, 0.16], roughness: 0.50 },
    MatInfo { name: "ice",       hardness: 1.5, cohesion: 0.40, albedo: [0.72, 0.80, 0.88], roughness: 0.25 },
    MatInfo { name: "structural alloy", hardness: 99.0, cohesion: 1.0, albedo: [0.48, 0.50, 0.54], roughness: 0.40 },
];

#[inline]
pub fn info(id: u8) -> MatInfo { PALETTE[(id as usize).min(7)] }
...
pub fn material_at(t: &Terrain, p: [f32; 3]) -> u8 {
    ...
    let d = 2.0;
    let slope = {
        let gx = (t.elevation(theta + d / hab.radius, z) - t.elevation(theta - d / hab.radius, z)).abs();
    ...
    if below > 4.0 {
        ...
        let v2 = fbm3(p[0] * s + 17.0, p[1] * s - 9.0, p[2] * s + 3.0, 4, hab.seed ^ 0x0E02);
    ...
    if below < reg_d + sed_d && flux > 0.55 && slope < 0.22 {
        let clay_n = fbm2(tf * 0.02, (z / hab.length) * NZ as f32 * 0.02, 3, 3, hab.seed ^ 0xC1A4);
        if clay_n > 0.45 {
            return id::CLAY;
        }
    }
    ...
    let warp = fbm3(p[0] * 0.008, p[1] * 0.008, p[2] * 0.008, 3, hab.seed ^ 0x57A7) * 12.0;
    let band = below + warp;
...
pub fn name(id: u8) -> &'static str { info(id).name }

pub fn albedo(id: u8) -> [f32; 3] { info(id).albedo }
