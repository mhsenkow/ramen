...
impl Edits {
    pub fn len(&self) -> usize { self.strokes.len() }

    pub fn strokes_slice(&self) -> &[Stroke] { &self.strokes }
    ...
    pub fn add(&mut self, c: [f32; 3], radius: f32, dig: bool, level: bool, up: [f32; 3]) {
        ...
        self.strokes.push(Stroke { c, radius, dig, level, up });
        // Register in every bucket the stroke can reach, so lookup is O(1).
        let r = if level { radius * 1.7 + 1.0 } else { radius + 1.0 };
    ...
    pub fn pop(&mut self) -> Option<Stroke> {
        ...
        for (i, st) in strokes.iter().enumerate() {
            let r = if st.level { st.radius * 1.7 + 1.0 } else { st.radius + 1.0 };
    ...
    pub fn apply(&self, p: [f32; 3], mut d: f32) -> f32 {
        if self.strokes.is_empty() { return d; }
        if let Some(ids) = self.index.get(&key(p)) {
            for &i in ids {
                ...
                if s.level {
                    ...
                    let ax = [rel[0] - s.up[0]*h, rel[1] - s.up[1]*h, rel[2] - s.up[2]*h];
                    ...
                    if s.dig { d = d.min(dist - s.radius); }
                    else { d = d.max(s.radius - dist); }
                }
            }
        }
        d
